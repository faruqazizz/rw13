...
class Group extends Backend{
  ...
  function _rules()
  {
    $this->form_validation->set_rules("group","*&nbsp;","trim|xss_clean|htmlspecialchars|required");
      ...
      foreach ($list as $rows) {
          ...
          $row[] = '
                    <div class="btn-group" role="group" aria-label="Basic example">
                        ...
                        <a href="'.url("group/copy_control/".enc_url($rows->id)).'" id="copy" class="btn btn-dark" title="Salin Akses">
                          <i class="mdi mdi-checkbox-multiple-marked"></i>Salin Akses
                        </a>
                      </div>
                   ';
  ...
  function add()
  {
    $this->db->query("ALTER TABLE auth_group AUTO_INCREMENT = 1");
  ...
  function copy_control($id = "")
  {
      if (!is_allowed('groups_access')) {
        ...
        exit();
          ...
          $group_id = dec_url($id);
          $source_permissions = $this->db->query("SELECT permission_id FROM auth_permission_to_group WHERE group_id = ".$group_id)->result_array();
          if ($this->model->find($group_id) && count($source_permissions)) {
              // Buat grup baru dengan nama dan deskripsi yang sama
              $source_group = $this->model->find($group_id);
              $save_data = [
                  "group"      =>  "New Group",
              ...
              // Salin permission
              $data = [];
              foreach ($source_permissions as $perms) {
                  $data[] = [
