    ...
    <div class="card m-b-30">
      ...
      <div class="card-body">
          ...
          <?php if ($id_group == 1): ?>
            ...
            <?php else: ?>
              <?php if (profile('id_group') <= 2): ?>
                <div class="form-group">
                    <label for="example-search-input" class="">Group</label>
                      <?=is_combo("auth_group","id_group","id","group",$id_group,"!=","1")?>
                </div>
              <?php else: ?>
                <div class="form-group <?= profile('id_group') == 3 ? 'd-none' : '' ?>">
                <label for="example-search-input" class="">Group</label>
                <input type="text" class="form-control" readonly value="<?= strtoupper($panggilgroup)?>">
                <input type="hidden" name="id_group" id="id_group" value="<?=$id_group?>">
            </div>
              <?php endif; ?>
          <?php endif; ?>
          ...
          <?php if ($button=="update"): ?>
            ...
            <p class="text-primary" style="font-size:12px">* Biarkan kosong jika tidak ingin mengganti password.</p>
          <?php endif; ?>
          ...
          <div class="form-group">
            ...
            <div class="form-group <?= profile('id_group') >= 2 ? 'd-none' : '' ?>">
                <label for="example-search-input" class="">Warga</label>
                <?=is_select_dua("warga","id_warga","id","no_ktp","nama_lengkap", $id_warga);?>
                <p class="text-primary" style="font-size:12px">* ID Warga tidak bisa diubah kecuali dihapus, dan buat baru di daftar warga.</p>
            </div>
