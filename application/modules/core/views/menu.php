            ...
            <li class="nav-item">
              <a class="nav-link" data-toggle="collapse" href="#menus<?=$menu->id_menu?>" aria-expanded="false" aria-controls="error">
                ...
                <?php foreach ($getSubMenu->result() as $sub_menu): ?>
                  <?php 
                  if(profile('id_warga')==3){
                    $url_sub_menu = ($sub_menu->type == "controller") ? site_url(ADMIN_ROUTE."/$sub_menu->controller") : site_url(ADMIN_ROUTE."/".$sub_menu->controller. enc_url(profile('id_warga')));
                  } else{
                    $url_sub_menu = ($sub_menu->type == "controller") ? site_url(ADMIN_ROUTE."/$sub_menu->controller") : $sub_menu->controller;
                  }
                    ?>
