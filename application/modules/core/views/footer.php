...
<footer class="footer">
  <div class="d-sm-flex justify-content-center justify-content-sm-between">
    <span class="text-muted text-center text-sm-left d-block d-sm-inline-block">Page rendered in <strong>{elapsed_time}</strong> seconds.<?= $this->config->item("author") ?> - <?= CI_VERSION; ?> Copyright © <?= date("Y") ?>. All rights reserved.</span>
    <span class="float-none float-sm-right d-block mt-1 mt-sm-0 text-center">UNM - <a href="https://faservice.github.io/">Faserv</a> <i class="ti-heart text-danger ml-1"></i></span>
  </div>
</footer>
<!-- partial -->
</div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    if ($(".text-editor").length) {
      ...
      function deleteImage(src) {
        $.ajax({
          ...
          type: "POST",
          url: "<?= url($this->uri->segment(2) . "/imageRemoveEditor") ?>",
          cache: false,
    ...
    $('.file-upload-default').on('change', function() {
      ...
      var nama_data = $(this).attr("name");
      var form_data = new FormData();
      form_data.append("file", file_data);
      form_data.append("nama_data", nama_data);
      $.ajax({
