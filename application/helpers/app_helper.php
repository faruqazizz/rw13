...
if(!function_exists('profile')) {
	function profile($field)
	{
	  ...
	  $qry = $ci->db->select("auth_user_to_group.id_user AS id_user,
	                          ...
	                          auth_user.id_warga AS id_warga,
...
if (!function_exists('is_document')) {
	function is_document($params = false, $class = "", $style = "")
	{
		$str = '';
		if ($params) {
			if (file_exists(FCPATH."_temp/uploads/img/$params")) {
				$str.='<a href="'.base_url().'_temp/uploads/img/'.$params.'" target="_blank">'.$params.'</a>';
			}else {
				$str.='<a href="'.base_url().'_temp/uploads/noimage.jpg" data-fancybox="gallery" title="No Image Available">
		              <img src="'.base_url().'_temp/uploads/noimage.jpg" alt="noimage" style="'.$style.'" class="'.$class.'" />
		            </a>';
...
if (!function_exists('is_select_like')) {
	function is_select_like($table, $attribute, $value, $label, $entry_value = "", $where_clause="", $like=true)
	{
		$ci=& get_instance();
		$str = "";
		if (!empty($where_clause)) {
			if($like){
				$ci->db->like($label,$where_clause);
			}else{
				$ci->db->not_like($label,$where_clause);
			}
        }
		$qry =  $ci->db->get($table);
		$str.= '<select class="form-control form-control-sm select2" data-placeholder=" -- Select -- " name="'.$attribute.'">';
...
if (!function_exists('is_select_dua')) {
	function is_select_dua($table, $attribute, $value, $label, $labeldua, $entry_value = "")
	{
		$ci=& get_instance();
		$str = "";
		$qry =  $ci->db->get($table);
		...
		$str.= '<option value=""></option>';
		foreach ($qry->result() as $row) {
			$str.='<option '.($row->$value == $entry_value ? "selected":"").' value="'.$row->$value.'">'.$row->$label.' | '.$row->$labeldua.'</option>';
		}
		$str.= '</select>';
		$str.='<div  id="'.$attribute.'"></div>';
...
if(!function_exists('is_combo')) {
	function is_combo($table,$id_name,$id_field,$name_field,$value,$tanda,$idnya)
	{
	  $ci=& get_instance();
	  $qry = $ci->db->get_where("$table",["id $tanda"=>"$idnya"]);
...
if (!function_exists('delete_user')) {
	function delete_user($id) {
	  $ci =& get_instance();
	//   if (profile('id_user') == 1) {
		$ci->db->query("delete from auth_user where id_user='".$id."'");
		$ci->db->query("delete from auth_user_to_group where id_user='".$id."'");
	// } else {
		// $ci->db->set('is_delete', '1');
		// $ci->db->where('id_user', $id);
		// $ci->db->update('auth_user');        
	//   }
	}
}
